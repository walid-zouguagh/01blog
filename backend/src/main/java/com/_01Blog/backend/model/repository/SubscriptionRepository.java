...
public interface SubscriptionRepository extends JpaRepository<Subscription, UUID> {
        ...
        @Query(value = """
                        SELECT EXISTS(SELECT 1 FROM subscriptions WHERE follower_id = :userId AND following_id = :dataUser)
                            """, nativeQuery = true)
        ...
        @Query(value = """
                        SELECT COUNT(DISTINCT s.follower_id) FROM subscriptions s WHERE s.following_id = :userId
                        """, nativeQuery = true)
        int countOfFollower(@Param("userId") UUID userId);

        @Query(value = """
                        SELECT COUNT(DISTINCT s.following_id) FROM subscriptions s WHERE s.follower_id = :userId
                        """, nativeQuery = true)
        int countOfFollowing(@Param("userId") UUID userId);

        @Query(value = """
                        SELECT s.follower_id FROM subscriptions s WHERE s.following_id = :userId LIMIT 10 OFFSET :offset
                        """, nativeQuery = true)
        List<UUID> getFollowers(@Param("userId") UUID userId, @Param("offset") int offset);

        @Query(value = """
                        SELECT s.following_id FROM subscriptions s WHERE s.follower_id = :userId LIMIT 10 OFFSET :offset
                        """, nativeQuery = true)
